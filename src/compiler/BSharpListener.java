...
public class BSharpListener extends BSharpBaseListener {
    ...
    private boolean isDoubleRegisterUsed = false;
    private boolean isLogicRegisterUsed = false;

    BSharpListener(String path) {
        filePath=path;
    }
    ...
    public void exitLogicalExpression(BSharpParser.LogicalExpressionContext ctx) {
        ...
        if (ctx.op != null) {
            ...
            isLogicRegisterUsed = updateRegisters(isLogicRegisterUsed, left, right);
        } else {
    ...
    private boolean updateRegisters(boolean isPrimaryRegUsed, String left, String right) {
        if (!isPrimaryRegUsed) {
            intermediateCode.add("MOV "+ "X " + "ACC");
            isPrimaryRegUsed = true;
        } else {
            if ((left.equals("X") && !right.equals("Y")) ||
                    (right.equals("X") && !left.equals("Y"))) {
                intermediateCode.add("MOV "+ "X " + "ACC");
            } else {
                intermediateCode.add("MOV "+ "Y " + "ACC");
                isPrimaryRegUsed = false;
            }
        }
        return isPrimaryRegUsed;
    }
